    ...
    <div className="w-full">
      ...
      <section className="relative pt-24 pb-12 md:pt-32 md:pb-24 overflow-hidden min-h-[90vh] flex flex-col justify-between bg-slate-100">
         ...
         <div className="max-w-7xl mx-auto w-full relative z-20 flex-1 flex flex-col">
            {/* Headline Overlay */}
            <div className="mt-8 md:mt-12 bg-white/95 backdrop-blur-md p-6 md:p-8 rounded-[2rem] max-w-lg shadow-2xl border border-white/50 relative z-30">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#111111] rounded-full mb-4">
                  <Award className="w-3 h-3 text-yellow-400" />
                  <span className="text-[10px] font-bold uppercase tracking-wider text-yellow-400">100+ Years of Excellence</span>
                </div>
                <h1 className="text-4xl md:text-5xl font-black leading-[1.1] tracking-tight text-[#111111]">
                    Mazboot Saaman, <br/>
            ...
            <div className="absolute inset-0 w-full h-full pointer-events-none">
              ...
              {/* Hotspot 1: Roof */}
              <div className="absolute top-[20%] left-[45%] md:left-[55%] pointer-events-auto group z-20">
                <div className="relative w-12 h-12">
                   <div className="absolute inset-0 bg-yellow-400 rounded-full animate-ping opacity-75"></div>
                   <div className="relative w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(255,200,0,0.6)] cursor-pointer hover:scale-110 transition-transform">
                     <Paintbrush className="w-5 h-5 text-[#111111]" />
                   </div>
                   
                   {/* Popover */}
                   <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-56 md:w-64 bg-white p-4 rounded-2xl shadow-2xl opacity-0 translate-y-4 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300">
                      <div className="font-bold text-[#111111] mb-1">Paints & Coatings</div>
                      <p className="text-xs text-gray-500 mb-3">Top brands like Dulux, Asian Paints & Indigo for exterior & interior.</p>
              ...
              {/* Hotspot 2: Windows / Kitchen */}
              <div className="absolute top-[50%] left-[65%] md:left-[75%] pointer-events-auto group z-20">
                <div className="relative w-12 h-12">
                   <div className="absolute inset-0 bg-yellow-400 rounded-full animate-ping opacity-75" style={{animationDelay: "0.5s"}}></div>
                   <div className="relative w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(255,200,0,0.6)] cursor-pointer hover:scale-110 transition-transform">
                     <Utensils className="w-5 h-5 text-[#111111]" />
                   </div>
                   {/* Popover */}
                   <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-56 md:w-64 bg-white p-4 rounded-2xl shadow-2xl opacity-0 translate-y-4 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300">
                      <div className="font-bold text-[#111111] mb-1">Kitchens & Hardware</div>
                      <p className="text-xs text-gray-500 mb-3">Premium fittings from Hafele, Hettich & Yale digital locks.</p>
                      <Link href="/products" className="text-xs font-bold text-[#D22027] flex items-center gap-1 hover:underline">Explore <ChevronRight className="w-3 h-3"/></Link>
                   </div>
                </div>
              </div>

              {/* Hotspot 3: Side Wall / Electrical */}
              <div className="absolute top-[65%] left-[25%] md:left-[35%] pointer-events-auto group z-20">
                <div className="relative w-12 h-12">
                   <div className="absolute inset-0 bg-yellow-400 rounded-full animate-ping opacity-75" style={{animationDelay: "1s"}}></div>
                   <div className="relative w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(255,200,0,0.6)] cursor-pointer hover:scale-110 transition-transform">
                   ...
                   <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-56 md:w-64 bg-white p-4 rounded-2xl shadow-2xl opacity-0 translate-y-4 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300">
                      ...
                      <p className="text-xs text-gray-500 mb-3">Wires, switches, and heating solutions from GreatWhite & Racold.</p>
                      <Link href="/products" className="text-xs font-bold text-[#D22027] flex items-center gap-1 hover:underline">Explore <ChevronRight className="w-3 h-3"/></Link>
                   </div>
                </div>
              </div>

              {/* Hotspot 4: Pipes / Bathroom */}
              <div className="absolute top-[45%] left-[80%] md:left-[90%] pointer-events-auto group z-20 hidden md:block">
                <div className="relative w-12 h-12">
                   <div className="absolute inset-0 bg-yellow-400 rounded-full animate-ping opacity-75" style={{animationDelay: "1.5s"}}></div>
                   <div className="relative w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(255,200,0,0.6)] cursor-pointer hover:scale-110 transition-transform">
                     <Droplets className="w-5 h-5 text-[#111111]" />
                   </div>
                   {/* Popover */}
                   <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-56 md:w-64 bg-white p-4 rounded-2xl shadow-2xl opacity-0 translate-y-4 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300">
                      <div className="font-bold text-[#111111] mb-1">Plumbing & Sanitary</div>
                      <p className="text-xs text-gray-500 mb-3">Complete piping and sanitaryware from Geberit, Vectus & Hindware.</p>
                      <Link href="/products" className="text-xs font-bold text-[#D22027] flex items-center gap-1 hover:underline">Explore <ChevronRight className="w-3 h-3"/></Link>
                   </div>
                </div>
              </div>

              {/* Hotspot 5: Floor/Driveway */}
              <div className="absolute top-[80%] left-[50%] md:left-[60%] pointer-events-auto group z-20">
                <div className="relative w-12 h-12">
                   <div className="absolute inset-0 bg-yellow-400 rounded-full animate-ping opacity-75" style={{animationDelay: "0.2s"}}></div>
                   <div className="relative w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(255,200,0,0.6)] cursor-pointer hover:scale-110 transition-transform">
            ...
            <div className="mt-auto self-center md:self-start bg-black/60 backdrop-blur-md px-5 py-3 rounded-full flex items-center gap-3 border border-white/20 relative z-30 shadow-xl mb-4">
               ...
               <span className="text-[11px] md:text-xs font-bold text-white tracking-widest uppercase">Hover the 5 hotspots to explore services</span>
            </div>
         </div>
      </section>
